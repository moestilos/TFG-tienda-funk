    ...
    >
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[
            { title: 'Camisetas', image: camisetaFront, hoverImage: camisetaHover, link: '/tiendacamisetas' },
            { title: 'Sudaderas', image: sudaderaFront, hoverImage: sudaderaHover, link: '/tiendasudaderas' },
            { title: 'Gorras', image: gorraFront, hoverImage: gorraHover, link: '/tiendagorras' }
          ].map((item, index) => (
            <Link to={item.link} key={index}>
              <div className="bg-black rounded-xl overflow-hidden border border-gray-700 hover:border-yellow-500/50 transition-all duration-300">
                <div className="relative aspect-square group overflow-hidden">
                  <img alt="collection" className="object-cover w-full h-full transition-all duration-500 absolute inset-0 transform scale-100 group-hover:scale-110 group-hover:opacity-0" src={item.image} />
                  <img alt="collection hover" className="object-cover w-full h-full transition-all duration-500 absolute inset-0 opacity-0 transform scale-110 group-hover:scale-100 group-hover:opacity-100" src={item.hoverImage} />
                </div>
                ...
                <div className="p-6 border-t border-gray-700 bg-black">
                  <div className="mb-4">
                    ...
                    <h2 className="text-white text-2xl mt-1 tracking-wide">{item.title}</h2>
                  </div>
